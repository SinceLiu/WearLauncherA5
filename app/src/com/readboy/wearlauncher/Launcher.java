...
import com.readboy.wearlauncher.battery.BatteryLevelImageView;
...
public class Launcher extends FragmentActivity implements BatteryController.BatteryStateChangeCallback,
    ...
    private static final int TURN_TO_MAIN_PAGE = 0;
    private static final int WAIT_IRWS = 1;
    ...
    private BatteryLevelImageView mNegativeScreenBatteryLevelImageView;
    private BatteryLevelImageView mLowPowerBatteryLevelImageView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLowPowerBatteryLevelImageView = (BatteryLevelImageView) mLowDialBaseLayout.findViewById(R.id.iv_battery);
        mLowDialBaseLayout.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (mBatteryLevel < lowPowerLevel && mPowerManager.isPowerSaveMode()) {
        ...
        mNegativeScreenBatteryLevelImageView = (BatteryLevelImageView) mNegativeView.findViewById(R.id.iv_battery);
        ...
        mViewpager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
            ...
            public void onPageSelected(int position) {
                //不在负一屏时，不刷新电量变化动画
                if (position == 0) {
                    mNegativeScreenBatteryLevelImageView.setBatteryController(mBatteryLevel);
                } else {
                    mNegativeScreenBatteryLevelImageView.cancelBatteryController();
                }
                if (position == 1) {
                    mAppView.moveToTop();
        ...
        waitIRWS();
        //checkCalibrationAndSyntheticTest();
    }
    ...
    public void waitIRWS() {
        if (rwm.getPersonalInfo() != null) {
            ...
            mHandler.removeMessages(WAIT_IRWS);
            mHandler.sendEmptyMessageDelayed(WAIT_IRWS, 100);
        }
    }

    private void checkCalibrationAndSyntheticTest() {
        String result = "";
        byte[] nvdata = NvJniItems.getInstance().getNv2499();//read 128 byte
        Log.e(TAG, "checkCalibrationAndSyntheticTest: nvdata[0] " + nvdata[0]);
        if (((char) nvdata[0]) != 'P') {
            result += "校准失败\n";
        }
        Log.e(TAG, "checkCalibrationAndSyntheticTest: nvdata[1] " + nvdata[1]);
        if (((char) nvdata[1]) != 'P') {
            result += "综测失败\n";
        }
    ...
    private void setMessageError(String str) {
        TextView textView = (TextView) findViewById(R.id.tv_msg_error);
        if (textView != null) {
    ...
    public void onBatteryLevelChanged(int level, boolean pluggedIn, boolean charging) {
        ...
        if (mBatteryLevel == -1 || mBatteryLevel != level) {
            ...
            if (mBatteryLevel < lowPowerLevel) {//low powe
                ...
                mLowPowerBatteryLevelImageView.setBatteryController(mBatteryLevel);
                ...
                mLowPowerBatteryLevelImageView.cancelBatteryController();
    ...
    private void dialResume() {
        if (mLowDialBaseLayout != null && mLowDialBaseLayout.getVisibility() == View.VISIBLE) {
            mLowDialBaseLayout.onResume();
            mLowDialBaseLayout.addChangedCallback();
            mLowPowerBatteryLevelImageView.setBatteryController(mBatteryLevel);
        }
